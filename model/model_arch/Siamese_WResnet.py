...
class Siamese_WResnet(nn.Module):

    def __init__(self, block, layers, num_classes=1):

        super(Siamese_WResnet, self).__init__()
        self.inplanes = 32 * block.expansion
        self.share_weight_features = self.subnetwork_share_weight(block, layers)
        self.features_0 = self.subnetwork_ind(block, layers)
        self.inplanes = 256 * block.expansion
        self.features_1 = self.subnetwork_ind(block, layers)
        self.dropout = nn.Dropout(p=0.5)
        self.fc_0 = nn.Linear(18432, num_classes)
        self.fc_1 = nn.Linear(18432, num_classes)
    ...
    def subnetwork_share_weight(self, block, layers):
    ...
    def subnetwork_ind(self, block, layers):
        features = nn.Sequential(
